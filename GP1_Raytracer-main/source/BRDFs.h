...
namespace dae
{
	namespace BRDF
	{
		...
		static ColorRGB Phong(float ks, float exp, const Vector3& l, const Vector3& v, const Vector3& n)
		{
			...
			const float cosAlpha			{ std::max(Vector3::Dot(reflectedRay,v), 0.f) };
		...
		{
			//todo: W3
			float dot{ 1.0f - std::max(0.0f, Vector3::Dot(h, v)) };

			return f0 + (ColorRGB(1.f, 1.f, 1.f) - f0) * (dot * dot * dot * dot * dot);
		}
		...
		static float NormalDistribution_GGX(const Vector3& n, const Vector3& h, float roughness)
		{
			...
			float divisor{ (dotNH * dotNH) * ((a * a) - 1.f) + 1.f };

			float result{ (a * a) / (PI * (divisor * divisor)) };
			return result;
		}
